...
void on_app_activate (GtkApplication * app, gpointer user_data)
{
	...
	GtkWidget * vbox1, * hbox1, * button1, * b_quit, * b_rotate;										// Création de la box & boutons
	...
	gtk_container_add (GTK_CONTAINER (my->window), vbox1);									// Ajout  de la box à la fenetre
		...
		b_rotate = gtk_button_new_with_label("Rotate");
	...
	gtk_box_pack_start (GTK_BOX (vbox1), my->image, TRUE, TRUE, 0);							// Ajoute le b_center à la vbox

	my->status = gtk_statusbar_new ();														// Instancie la status bar
	gtk_box_pack_start (GTK_BOX (vbox1), my->status, FALSE, FALSE, 0);						// Ajoute le b_bottom à la vbox
